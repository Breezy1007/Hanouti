...
export default function StorePage() {
  ...
  useEffect(() => {
    ...
    if (s) {
      ...
      if (store.id === id) {
        setStore(store);
        if (p) setProducts(JSON.parse(p).filter(prod => prod.available));
      }
    }
    setLoading(false);
  }, [id]);
  ...
  const sendOrder = () => {
    ...
    const orders = JSON.parse(localStorage.getItem("hanouti_orders") || "[]");
    orders.push({ customer_name: name, customer_phone: phone, items: cart, total, date: new Date().toISOString() });
    localStorage.setItem("hanouti_orders", JSON.stringify(orders));
  ...
  if (loading) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", background: "#f8f5f0" }}>
      <div style={{ textAlign: "center", fontFamily: "Tajawal, sans-serif" }}>
        ...
        <p style={{ color: "#8a7a6a" }}>جاري التحميل...</p>
      </div>
    </div>
  );
  ...
  if (!store) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", background: "#f8f5f0" }}>
      <div style={{ textAlign: "center", fontFamily: "Tajawal, sans-serif" }}>
        <div style={{ fontSize: 40, marginBottom: 16 }}>😕</div>
        <p style={{ color: "#8a7a6a" }}>المتجر ما كاينش</p>
      </div>
    </div>
  );
      ...
      <div className="root">
        <div className="header" style={{ borderBottom: `3px solid ${accent}` }}>
          <div className="store-info">
            <div className="store-avatar" style={{ background: `${accent}20` }}>🏪</div>
            <div>
        ...
        <div className="products">
          {ordered && <div className="success-banner">✅ تم إرسال طلبك! سيتواصل معك المتجر قريباً</div>}
          {products.length === 0 ? (
            <div className="empty"><p style={{ fontSize: 48 }}>📦</p><p>ما كاين حتى منتج دابا</p></div>
          ) : (
            <div className="products-grid">
              {products.map(p => (
                <div key={p.id} className="product-card">
                  ...
                  <button className="add-btn" style={{ background: accent }} onClick={() => addToCart(p)}>+ أضف</button>
                </div>
              ))}
            </div>
          )}
        ...
        {showCart && (
          <div className="modal-overlay" onClick={() => setShowCart(false)}>
            <div className="modal" onClick={e => e.stopPropagation()}>
              ...
              <div className="cart-total" style={{ color: accent }}>المجموع: {total} درهم</div>
              <div className="order-form">
                <input placeholder="اسمك الكامل *" value={name} onChange={e => setName(e.target.value)} className="order-input" />
                <input placeholder="رقم الهاتف *" value={phone} onChange={e => setPhone(e.target.value)} className="order-input" dir="ltr" type="tel" />
                <button className="order-btn" style={{ background: accent }} onClick={sendOrder} disabled={!name || !phone}>
                  📱 اطلب عبر واتساب
                </button>
              </div>
            </div>
        ...
        {cartCount > 0 && !showCart && (
          <button className="float-cart" style={{ background: accent }} onClick={() => setShowCart(true)}>
